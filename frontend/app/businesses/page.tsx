...
// Generate business profile URL: /business/{id}
const getBusinessProfileUrl = (business: any): string => {
              ...
              <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
                  {businesses.map((business, index) => {
                    ...
                    // Get random color for category badge based on category ID
                    const categoryColor = business.category?.id 
                      ? CATEGORY_COLORS[business.category.id % CATEGORY_COLORS.length]
                      : CATEGORY_COLORS[0];
                    
                    return (
                      <div key={business.id} className="bg-white rounded-lg border border-gray-200 overflow-hidden hover:shadow-lg transition group">
                        {/* Image */}
                        <div className="relative h-44 bg-gradient-to-br from-gray-100 to-gray-200">
                          {business.cover_image || business.featured_image ? (
                            <img src={getImageUrl(business.cover_image || business.featured_image)} alt={business.name} className="w-full h-full object-cover" />
                          ) : (
                          ...
                          {/* Ranking Badge */}
                          {rankNumber <= 20 && (
                            <div className="absolute top-3 left-3 bg-gradient-to-br from-yellow-400 to-orange-500 text-white px-2.5 py-1 rounded-lg font-bold text-sm shadow-lg">
                              #{rankNumber}
                            </div>
                          )}
                          ...
                          {business.category && (
                            <div className="absolute top-3 right-3">
                              <span className={`px-2.5 py-1 rounded-lg text-xs font-bold shadow ${categoryColor.bg} ${categoryColor.text}`}>
                          ...
                          <button
                            ...
                            className="absolute bottom-3 right-3 w-8 h-8 bg-white rounded-full flex items-center justify-center shadow-lg hover:scale-110 transition"
                          >
                            <svg className={`w-4 h-4 ${isSaved ? 'fill-red-500 text-red-500' : 'text-gray-400'}`} fill={isSaved ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div className="p-4">
                          ...
                          <div className="flex items-center gap-2 text-xs text-gray-600 mb-2">
                            <div className="flex items-center gap-0.5">
                              ...
                              <span className="font-semibold text-gray-900">{Number(business.rating || 0).toFixed(1)}</span>
                              <span className="text-gray-400">({business.review_count || 0} reviews)</span>
                            </div>
                          </div>
                          ...
                          {business.area && (
                            <div className="flex items-center gap-1 text-xs text-gray-600 mb-3">
                              ...
                              <span className="truncate">{business.area.name}, Patna</span>
                            </div>
                          )}
                          ...
                          <div className="flex flex-wrap gap-1.5 mb-3">
                            {business.is_open_now && (
                              <span className="text-xs px-2 py-0.5 bg-green-100 text-green-700 rounded font-medium">● Open Now</span>
                            )}
                            {business.has_menu && (
                              <span className="text-xs px-2 py-0.5 bg-orange-100 text-orange-700 rounded font-medium">Has Menu</span>
                            )}
                            {business.accepts_bookings && (
                              <span className="text-xs px-2 py-0.5 bg-purple-100 text-purple-700 rounded font-medium">Bookings</span>
                            )}
                          </div>
                          ...
                          <div className="flex justify-end">
                            <Link
                              href={getBusinessProfileUrl(business)}
                              className="inline-flex items-center gap-1.5 px-4 py-1.5 border-2 border-blue-600 text-blue-600 rounded-lg text-xs font-bold hover:bg-blue-50 transition"
                            >
                              View Profile
