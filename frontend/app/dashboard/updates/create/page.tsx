...
function CreateUpdatePageContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      await userUpdateApi.create({ ...formData, business_id: Number(selectedBusiness) });
  ...
  return (
    <DashboardLayout pageTitle="Create Update" pageSubtitle="Share news, offers, or announcements" showSaveButton={false}>
      <div className="p-4 sm:p-6 lg:p-8 max-w-3xl">
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            <label className="block text-sm font-medium text-gray-700 mb-2">Select Business *</label>
            <select value={selectedBusiness} onChange={(e) => setSelectedBusiness(e.target.value)} className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500" required>
